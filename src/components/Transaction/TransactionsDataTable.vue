...
export default defineComponent({
  ...
  setup (props, { emit, root }) {
    ...
    async function downloadReceipt (item: Transaction) {
      ...
      const receipt = await PaymentService.postReceipt(item.id, currentAccount.id, item.createdOn)
    ...
    const displayDate = (val: string) => {
      ...
      return CommonUtils.formatDisplayDateInPacific(date, 'MMMM DD, YYYY')
    }

    const displayTime = (val: string) => {
      const date = moment.utc(val).toDate()
      return CommonUtils.formatDisplayDateInPacific(date, 'h:mm A')
    }
